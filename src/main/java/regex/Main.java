...
public class Main {
    ...
    public static boolean checkForPassword(String str, int minLength) {
        ...
        final boolean capital = Pattern.matches(".*[A-Z].*", str);
        final boolean lower = Pattern.matches(".*[a-z].*", str);
        final boolean digit = Pattern.matches(".*[0-9].*", str);
        final boolean length = Pattern.matches(String.format(".{%d,}", minLength), str);
        // as needed, modify this code.
        return capital && lower && digit && length;
    }
    ...
    public static List<String> extractEmails(String str) {
        ...
        final Pattern pattern = Pattern.compile("[\\w]([\\w-.+][\\w-+]|[\\w-+][\\w-.+]|[\\w-+])*@(mail.utoronto.ca|utoronto.ca)");
